  ...
  public final class ScreenSharingCapture {
    ...
    public func start(
      ...
      sender: ScreenSharingFrameSender, metrics: ScreenSharingMetrics
    ) async throws {
      ...
      try await startResolved(
        ...
        sender: sender, metrics: metrics, generation: generation)
    }
    ...
    public func start(
      ...
      try Task.checkCancellation()
      try await startResolved(
        filter: pickedFilter, configuration: configuration, sender: sender, metrics: metrics, generation: generation)
    }

    private func startResolved(
      filter: SCContentFilter, configuration: ScreenSharingVideoConfiguration,
      sender: ScreenSharingFrameSender, metrics: ScreenSharingMetrics, generation: Int
    ) async throws {
      ...
      let output = CaptureOutput(sender: sender, metrics: metrics, copySurface: copySurface) { [weak self] message in
        Task { @MainActor in
  ...
  private final class CaptureOutput: NSObject, SCStreamOutput, SCStreamDelegate, @unchecked Sendable {
    ...
    init(
      sender: ScreenSharingFrameSender, metrics: ScreenSharingMetrics, copySurface: Bool,
    ...
    func stream(_ stream: SCStream, didOutputSampleBuffer sampleBuffer: CMSampleBuffer, of type: SCStreamOutputType) {
      ...
      do {
        ...
        sender.push(ScreenSharingVideoFrame(pixelBuffer: frame, timestampNs: time.value))
      } catch {
