...
final class ScreenSharingSourceIdleMonitor: @unchecked Sendable {
  ...
  /// diagnostic override.
  static let defaultThresholdNs: Int64 = 100_000_000
  static let legacyThresholdNs: Int64 = 500_000_000
  static let maximumQuickResubmissions = 4
  static let slowResubmissionIntervalNs: Int64 = 2_000_000_000
  let thresholdNs: Int64
  ...
  func recordSubmission(timestampNs: Int64, nowNs: Int64) -> Bool {
    lock.withLock {
  ...
  func recordEncoded(timestampNs: Int64) {
    lock.withLock {
  ...
  var latestEncodedNs: Int64? { lock.withLock { lastEncodedNs } }
  var latestSubmittedNs: Int64? { lock.withLock { lastSubmittedNs } }

  /// Evaluations performed (each is one wake of the notifier loop).
  var evaluationCount: Int { lock.withLock { evaluations } }
  private var evaluations = 0

  func evaluate(nowNs: Int64) -> Evaluation {
    lock.withLock {
      evaluations += 1
...
@MainActor
final class ScreenSharingSourceIdleNotifier {
  ...
  var hasPendingNotice: Bool { pendingNotice != nil }
...
final class ScreenSharingDeliveryAudit: @unchecked Sendable {
  private let lock = NSLock()
  ...
  static let recentDecodeCapacity = 64

  var latestDecodedTimestampNs: Int64? { lock.withLock { latestDecodedNs } }
  var pendingTargetNs: Int64? { lock.withLock { targetNs } }
  ...
  var targetMetAtTimestampNs: Int64? { lock.withLock { targetMetAtNs } }

  func decoded(sourceTimestampNs: Int64, nowNs: Int64 = ScreenSharingMetrics.nowNs) {
    lock.withLock {
  ...
  func noticed(latestTimestampNs: Int64) -> Bool {
    lock.withLock {
      recovering = false
      targetMetAtNs = nil
...
@MainActor
final class ScreenSharingDeliveryVerifier {
  ...
  static let defaultGrace: Duration = .milliseconds(100)
  static let defaultGraceExtensions = 4
  static let legacyGrace: Duration = .milliseconds(500)
  static let legacyGraceExtensions = 0
  static let maximumGraceExtensions = 10
  ...
  func noticed(latestTimestampNs: Int64) {
  ...
  func recoveryChanged(_ event: ScreenSharingRefreshSignal.Event) {
