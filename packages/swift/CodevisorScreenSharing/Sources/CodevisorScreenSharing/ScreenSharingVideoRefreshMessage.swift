...
enum ScreenSharingVideoRefreshMessage: Sendable, Equatable {
...
final class ScreenSharingRefreshSignal: @unchecked Sendable {
  ...
  func onChange(_ callback: @escaping @Sendable (Event) -> Void) {
  ...
  var keyframeGeneration: UInt64 { lock.withLock { generation } }
  func request(resetDecoder: Bool = false) { change(to: true, resetDecoder: resetDecoder) }
  ...
  @discardableResult
  func requestUnlessPending() -> Bool {
  ...
  func decodedKeyframe(generation: UInt64) { change(to: false, completedGeneration: generation) }
  func consumeDecoderReset() -> Bool {
    lock.withLock {
...
final class ScreenSharingEncoderRefreshRequest: @unchecked Sendable {
  private let lock = NSLock()
  private var pending = false
  func request() { lock.withLock { pending = true } }
  func consume() -> Bool {
    lock.withLock {
...
struct ScreenSharingRefreshRateLimit {
  static let intervalNs: Int64 = 100_000_000
  private var lastAllowed: Int64?

  mutating func allow(nowNs: Int64) -> Bool {
...
@MainActor
final class ScreenSharingRefreshRequester {
  ...
  func update(_ event: ScreenSharingRefreshSignal.Event) {
...
struct ScreenSharingRefreshFrameStore {
  ...
  mutating func capture(_ frame: ScreenSharingVideoFrame) -> ScreenSharingVideoFrame? {
  ...
  mutating func refresh(nowNs: Int64) -> ScreenSharingVideoFrame? {
  ...
  mutating func clear() { frame = nil }

  var isHolding: Bool { frame != nil }
