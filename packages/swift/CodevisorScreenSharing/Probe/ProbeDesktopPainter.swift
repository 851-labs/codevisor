...
final class ProbeDesktopPainter: @unchecked Sendable {
  let pattern: CGImage
  let fps: Int
  ...
  init(pattern: CGImage, fps: Int) { self.pattern = pattern; self.fps = fps }

  static func make(width: Int, height: Int, fps: Int) throws -> ProbeDesktopPainter {
  ...
  func draw(in context: CGContext, bounds: CGRect, sequence: Int, responses: Int) {
