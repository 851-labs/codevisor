...
- The package is split at a WebRTC-free seam. `ScreenSharingCore` (`packages/swift/ScreenSharingCore`) holds frames, the mailbox, metrics, the input/control/clipboard messages, the clipboard transfer, the `ScreenSharingMessageChannel` contract (one negotiated data channel today, an in-memory pair in tests) and the `ScreenSharingViewingSession` contract a viewer renders from. `CodevisorScreenSharing` re-exports it and adds, by stage folder, `Capture/` (ScreenCaptureKit), `Codecs/` (VideoToolbox), `Transport/` (the WebRTC `ScreenSharingSender` and `ScreenSharingReceiver` over a shared `ScreenSharingPeer` base, with their recovery objects and `ScreenSharingPeerOptions`) and `Render/` (the Metal renderer, which draws biplanar YCbCr and packed BGRA frames). The receiver is itself the `ScreenSharingViewingSession` a viewer renders from; `VNCScreenSharingSession` in `CodevisorCoreMac` is the other implementation, over the WebRTC-free `ScreenSharingRFB` protocol target (see [the VNC viewer plan](../../../docs/plans/vnc-viewer.md)). Experiment-only instrumentation lives in `apps/screen-sharing-rig/Sources/ScreenSharingDiagnostics`. The viewer feature in `CodevisorCoreMac` is a Composable Architecture reducer over a `ScreenSharingViewerBackend` (the native backend owns signaling) with the control lease as a child reducer over a `ScreenSharingEndpointClient`; input events stay on the data plane inside the endpoint. See [the composable architecture plan](../../../docs/plans/screen-sharing-composable-architecture.md).
...
swiftlint lint --strict --quiet packages/swift/CodevisorScreenSharing
bun run build:macos
bun run build:ios
```
...
Regenerate with `node scripts/build-webrtc.ts`; audit `artifacts/notices-macos/LICENSE.md` and `artifacts/notices-ios/LICENSE.md` against the recorded target graphs before copying them to `Sources/CodevisorScreenSharing/Resources/`. The main WebRTC source revision, dependency revisions, GN arguments and build tools are recorded in the recipe output. These notices cover the native macOS and iOS configurations; Mac Catalyst is not a Codevisor target.
