...
struct ScreenSharingPaneMappingTests {
  @Test func nativePaneRoundTripsOnlyPresentationPreferences() throws {
    let id = UUID()
    let preferences = ScreenSharingPanePreferences(preferredDisplayId: "display-uuid", fitToWindow: false)
    ...
    #expect(Set(fields.keys) == ["schemaVersion", "preferredDisplayId", "fitToWindow"])
  ...
  @Test func unsupportedMetadataDoesNotDiscardSiblingPanes() throws {
    ...
    var record = ServerWorkspacePane(
      ...
      paneType: "screen-sharing", title: "Screen Sharing", metadata: #"{"schemaVersion":2,"fitToWindow":true}"#,
