...
// Vendored upstream Tree-sitter C (runtime 0.26.13 and generated grammars, see
// CodeHighlighter/Vendor/manifest.json) is never hand-patched: it is regenerated by
// scripts/vendor-tree-sitter.py. Upstream narrows size_t/uintptr_t to 32 bits on purpose (wasm scanner
// addresses, serialization lengths bounded by TREE_SITTER_SERIALIZATION_BUFFER_SIZE) and still does at
// runtime v0.27.0 and every grammar's default branch, so exactly that one warning is off for these
// third-party targets; every other warning stays an error.
let vendoredTreeSitterCSettings: [CSetting] = strictCSettings + [.disableWarning("shorten-64-to-32")]
...
let package = Package(
  ...
  targets: [
    ...
    // The one call into WebRTC's deprecated process-wide field-trial API, isolated in Objective-C so
    // its deprecation can be acknowledged for exactly that call (see the .m file).
    .target(
      name: "WebRTCProcessFieldTrials",
      dependencies: [.product(name: "WebRTC", package: "WebRTC")],
      path: "ScreenSharing/Sources/WebRTCProcessFieldTrials",
      cSettings: strictCSettings
    ),
    .target(
      name: "ScreenSharingWebRTC",
      dependencies: ["ScreenSharing", "WebRTCProcessFieldTrials", .product(name: "WebRTC", package: "WebRTC")],
    ...
    .testTarget(
      ...
      // Real-server RFB recordings replayed by RFBRecordingTests (see Fixtures/README.md).
      resources: [.copy("RFB/Fixtures")],
      swiftSettings: strictSwiftSettings
    ),
    .testTarget(
    ...
    .target(
      ...
      cSettings: vendoredTreeSitterCSettings + [.headerSearchPath("lib/src")]
    ),
    .target(
      ...
      publicHeadersPath: "include",
      cSettings: vendoredTreeSitterCSettings + [.headerSearchPath("typescript/typescript/src")]
    ),
    .target(
