...
The simulator stays alive across dev-runner restarts. Stopping `ios-simulator` shuts down and deletes only its owned device. Device files use CoreSimulator's default location for DeviceHub compatibility; the worktree's ownership manifest lives in `tmp/runtime/ios-simulator.json`. Do not manually boot, delete, or reuse another worktree's device.
