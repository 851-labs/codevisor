...
`dev:all` builds and launches both native apps against one shared set of macOS-owned local, remote, and cloud development services. Use it for cross-device synchronization testing. It accepts the same `CODEVISOR_IOS_SIMULATOR` override as `dev:ios`.
