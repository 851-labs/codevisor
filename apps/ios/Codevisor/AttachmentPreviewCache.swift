...
final class AttachmentImageStore {
    ...
    /// Returns a validator-matched preview, fetching and downsampling only on
    /// a true miss. Concurrent views requesting the same source share one task.
    ...
    private func decodeDiskPayload(
        ...
        let image = await Task.detached(priority: .userInitiated) {
            UIImage(data: payload.thumbnail)
        }.value
    ...
    private func storeInMemory(_ preview: AttachmentPreviewImage, key: String) {
        let cost = preview.image.cgImage.map { $0.bytesPerRow * $0.height }
