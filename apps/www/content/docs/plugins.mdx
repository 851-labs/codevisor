...
| `CODEVISOR_PLUGIN_CONTEXT_SECRET` | Key for verifying `X-Codevisor-Context-Signature` |
...
Any local process can reach `127.0.0.1:$PORT` and send its own `X-Codevisor-Context`. Codevisor signs
the header so a plugin can reject context it did not send. `X-Codevisor-Context-Signature` is the
lowercase hex HMAC-SHA256 of the exact `X-Codevisor-Context` header value (the base64 text, before
decoding), keyed with the `CODEVISOR_PLUGIN_CONTEXT_SECRET` string. Each plugin gets its own secret, so
one plugin cannot sign context for another. The secret stays the same across plugin restarts. It
changes when the Codevisor server restarts, so read it from the environment and don't store it.
Compare signatures in constant time:

```js
import { createHmac, timingSafeEqual } from "node:crypto"

const verifyContext = (headers) => {
  const secret = process.env.CODEVISOR_PLUGIN_CONTEXT_SECRET
  const context = headers["x-codevisor-context"]
  const signature = headers["x-codevisor-context-signature"]
  if (!secret || typeof context !== "string" || typeof signature !== "string") return false
...
Verification is optional. It matters when the context grants access, for example when a plugin
reads or writes files under `cwd`. Older Codevisor servers do not set the variable, and
a WebSocket upgrade accepted through the loopback exemption carries no context headers.
...
`toolName`, and the available `workspaceId` and `cwd`. Verify them as described in
[Verify the context](#verify-the-context). Return JSON or plain text with a 2xx status;
