...
server. Codevisor starts it on demand, assigns it a loopback port, and proxies its panes and tools to
authenticated clients and agent sessions.
...
- `idleTimeoutSeconds`, which defaults to `300`; set it to `0` to keep the process alive.
...
Plugins start lazily on the first pane or tool request. Codevisor waits up to 15 seconds for
readiness, shuts idle processes down, and restarts crashed processes with backoff. Five consecutive
crashes leave the plugin in `failed` until an explicit restart. The proxy returns `502` when the
process cannot be reached and `504` when a request exceeds 30 seconds. An open pane WebSocket keeps
the process active.
