        ...
        .plugin(tauri_plugin_single_instance::init(|app, args, _cwd| {
            if let Some(window) = app.get_webview_window("main") {
                ...
                if let Some(route) = route_from_args(&args) {
                    let script = format!("window.history.pushState(null, '', {}); window.dispatchEvent(new PopStateEvent('popstate')); window.dispatchEvent(new Event('herdman-route-changed'));", serde_json::to_string(&route).unwrap_or_else(|_| "\"/\"".to_string()));
