// HERDMAN-PATCH: removed `import AppIntents` (AppEnum blocks removed below)
import Cocoa
import SwiftUI
import GhosttyKit
...
// HERDMAN-PATCH: removed AppEnum conformance block (upstream Shortcuts/App Intents support; conflicts with HerdMan's default-MainActor isolation and HerdMan ships no App Intents)
...
// HERDMAN-PATCH-BEGIN: removed `Ghostty.Input.Key: AppEnum` conformance block
// (upstream L1179-1316; Shortcuts/App Intents support conflicts with HerdMan's
// default-MainActor isolation and HerdMan ships no App Intents). The `allCases`
