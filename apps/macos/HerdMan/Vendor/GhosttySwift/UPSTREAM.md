...
Upstream Ghostty's macOS surface/input layer, vendored so HerdMan's terminal
...
rewritten as `Features/Terminal/HerdManGhosttyApp.swift`), the SwiftUI wrapper
...
Every deviation from upstream is wrapped in `HERDMAN-PATCH-BEGIN/END` markers
(or a single-line `HERDMAN-PATCH:` note for deletions). Find them all:

```sh
grep -rn "HERDMAN-PATCH" apps/macos/HerdMan/Vendor/GhosttySwift
```

Patch classes (12 files):
1. **Explicit imports** (`Foundation`, `AppKit`, `Combine`, `os`, `SwiftUI`) —
   HerdMan builds with `SWIFT_UPCOMING_FEATURE_MEMBER_IMPORT_VISIBILITY`;
...
3. **AppEnum/App Intents removed** (Ghostty.Input) — conflicts with HerdMan's
...
4. Build; new `MemberImportVisibility` or isolation errors mean new patch-class-1
   entries.
5. Verify per `apps/macos/HerdMan/Features/Terminal/README.md`.
