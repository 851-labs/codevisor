// The vanilla-JS contract for reading the active resolved theme, plus the two
// adapters that produce one. React context simply carries the current
// ThemeSource; the override precedence (prop > provider) falls out because a
// `theme` prop constructs a local fixedSource. Copied from pierre's diffshub
// app (lib/theme/ThemeSource.ts) pending its move into @pierre/theming.
...
// theme. It deliberately does not know about theme-name pairs. Chrome and the
// terminal only need this shape.
export interface ThemeSource {
  subscribe(listener: () => void): () => void
  getSnapshot(): ActiveThemeSnapshot
}

// The name-selection extension: selected light/dark theme names plus the
// active scheme, for consumers that need names rather than resolved objects
// (e.g. syntax highlighting that loads themes itself).
...
export function requireThemeValueName(value: ThemeValue): string {
  ...
  if (name == null || name === "") {
    throw new Error("ThemeInput ThemeLike values must include a `name`")
  }
  return name
}
...
// flight), we return the last non-undefined theme we saw, so the chrome never
// flashes the default palette. The scheme always follows the controller
// immediately so a mode flip is visible at once even while the new theme
// object loads.
...
// provider mode. Names are lazy-resolved through the resolver ("load on
// demand") and the source notifies its subscribers once the resolve settles.
// The last-resolved value is kept so a cold name never reports undefined after
// it has loaded once.
export function fixedSource(
  ...
  // for a name it follows the provider mode until the object resolves, then
  // the resolved object's type wins (matching the controller's resolved-object
  ...
  // Pull the name (if any) for the slot we should show, plus an
  // already-resolved object when the input carried one directly.
