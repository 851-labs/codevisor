/**
 * deriveChromeTokens derives the contrast-heavy "chrome" token set — the
 * opinionated app palette (most-legible foreground, derived muted text,
 * color-mix surfaces, luminance-based status tints).
 *
 * Surfaces come resolved from normalizeThemeColors, but the foreground
 * candidate list is read from the raw theme colors in design-intent order, so
 * the contrast pass can compare sideBar.foreground vs editor.foreground vs
 * theme.fg against each other (the resolved/collapsed sidebar fg has already
 * discarded that).
 *
 * Copied from pierre's diffshub app (lib/theme/deriveChromeTokens.ts).
 */
...
// unparseable, or fails the contrast bar — letting the call site fall back to
// a derived muted.
...
export function deriveChromeTokens(theme: ThemeLike): ChromeTokens | undefined {
  ...
  // Resolved surfaces come from normalizeThemeColors; the raw colors map
  // drives the design-intent foreground candidate list.
  ...
  const focusOutline = resolved["list.focusOutline"]
  const accent =
    focusOutline != null && sidebarBg != null
      ? (colorUtils.compositeOverBg(focusOutline, sidebarBg) ?? focusOutline)
      : fg
  ...
  // Opaque chrome border (header bottom, sidebar edge); shares BORDER_MIX with
  // the separator so the two read as one system.
  const borderOpaque = `color-mix(in srgb, ${fg} ${BORDER_MIX}%, ${sidebarBg ?? "transparent"})`
  ...
  // separator can't drift; only when the palettes genuinely diverge derive it
  // from the editor surface so it contrasts the content body.
  const separator =
    editorBg == null || colorUtils.surfacesMatch(editorBg, sidebarBg)
      ? borderOpaque
      : `color-mix(in srgb, ${editorFg} ${BORDER_MIX}%, ${editorBg})`
