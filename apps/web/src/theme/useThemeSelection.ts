...
// return the same reference on every call or it would loop. Hence a
// module-level singleton rather than a fresh object literal.
...
// override-only provider). It never notifies and always reports
// FALLBACK_STATE, so useThemeController can be called unconditionally — the
// hook count stays constant whether or not a controller exists.
...
// Reads the controller behind the provider: the current mode + theme names,
// the catalogs, and setters. Under an override-only provider (no controller)
// it returns empty catalogs and no-op setters so consumers stay safe.
