...
import type { ActiveThemeSnapshot, ThemeSource } from "./ThemeSource"

export const ThemeSourceContext = createContext<ThemeSource | undefined>(undefined)

// Carries the controller behind the provider source so the selection hook can
// read selection + setters. Undefined under an override-only provider (a
// fixedSource has no controller).
...
// useSyncExternalStore can keep a stable reference and avoid a render loop
// (the source may allocate a fresh object on every getSnapshot call).
