...
import { controllerSource, fixedSource, type ThemeInput } from "./ThemeSource"
import {
...
// default, follows-the-selector source. `theme=` (no controller) makes a
// frozen override source that shadows whatever provider source is above it —
// its colorScheme is read from the parent provider so a { light, dark } pair
// picks the slot matching the current mode. Precedence falls out of context
// nesting: nearest provider wins, and a per-component `theme` prop (in the
// prop hooks) bypasses the context entirely.
