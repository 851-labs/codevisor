...
/// The platforms with an App Store screenshot runner.
export type CapturePlatform = "macos" | "ios"

/// `--help` short-circuits parsing: every runner prints usage and exits
/// before it reads any other option.
export interface CaptureHelpRequest {
  help: true
}
...
/// iOS always parses a device (defaulting to the iPhone) and may pin a runtime.
export interface IosCaptureOptions extends CaptureOptions {
  device: string
}

export type ParsedCaptureOptions = CaptureHelpRequest | CaptureOptions

/// One App Store screenshot slot: the pixel size a scene must match, and the
/// display scales that produce it.
...
/// The subset of an `xcresulttool export attachments` manifest entry the
/// screenshot runners rely on.
...
export interface ScreenshotAttachmentTest {
...
export interface GalleryImage extends ScreenshotDimensions {
  file: string
  device: string
  scene: string
  appearance: string
...
export function parseCaptureOptions(
  ...
  root: string,
  platform: "ios"
): CaptureHelpRequest | IosCaptureOptions
export function parseCaptureOptions(
  args: readonly string[],
  root: string,
...
export function parseCaptureOptions(
  ...
  for (let index = 0; index < args.length; index++) {
    // The loop bound keeps this in range; the compiler cannot see that.
    const flag = args[index]!
    if (flag === "--help") return { help: true }
    if (!flags.includes(flag)) throw new Error(`Unknown option: ${flag}`)
    const value = args[++index]
    if (!value || value.startsWith("--")) throw new Error(`Missing value for ${flag}`)
    // `flags` above accepts only these four spellings.
    const key = flag.slice(2) as "output" | "appearance" | "device" | "runtime"
    options[key] = flag === "--output" ? resolve(root, value) : value
  }
...
export function selectedAppearances(options: Pick<CaptureOptions, "appearance">): string[] {
...
export function screenshotAttachments(
  manifest: readonly ScreenshotAttachmentTest[]
): SelectedScreenshot[] {
  ...
  return scenes.map((scene) => {
    ...
    // The length check above guarantees this element exists.
...
export function pngDimensions(bytes: Buffer, device: ScreenshotDevice): ScreenshotDimensions {
  if (
    bytes.length < 24 ||
...
export function gallery(images: readonly GalleryImage[]): string {
  const cards = images
    .map(
