...
import { summarizeTranscriptPerformance } from "./transcript-performance-summary.ts"
import type { TranscriptRecord } from "./transcript-performance-summary.ts"

test("reports independent timing distributions and the latest viewport", () => {
  const records: TranscriptRecord[] = [20, 2, 10, 4].map((durationMS) => ({
    name: "ios.mount",
    durationMS
  }))
  records.push(
