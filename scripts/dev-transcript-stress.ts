...
/// The dev endpoint's request body: seed creates a chat, chunk and finish
/// target an existing fixture.
interface TranscriptStressRequest {
  action: string
  title?: string
  folderPath?: string
  turns?: number
...
let body: TranscriptStressRequest
if (action === "seed") {
  ...
  throw new Error(
    "Usage: node scripts/dev-transcript-stress.ts seed [preset] | chunk <sessionId> <text|--stdin> | finish <sessionId>"
  )
}
