...
/// The workspace package.json fields this test reads. A workspace that builds
/// with `tsc -b` always defines a typecheck script alongside it.
interface WorkspaceManifest {
  scripts?: { build?: string; typecheck: string }
}

/// `execFileSync` attaches the child's exit status and captured output to the
/// error it throws.
...
for (const command of commands) {
  test(`typecheck leaves runtime output and build metadata alone: ${command}`, (t) => {
    ...
    const run = (args: readonly string[]) =>
      execFileSync(process.execPath, [tsc, ...args], { cwd: directory })
    ...
    assert.throws(
      ...
      (error) => {
        ...
        assert.match(failure.stdout.toString(), /TS2322/)
        return true
      }
    )
  })
