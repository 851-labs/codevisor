...
import { gallery, pngDimensions, screenshotAttachments } from "./screenshots-lib.ts"
import type { CapturePlatform, GalleryImage, ScreenshotDevice } from "./screenshots-lib.ts"
...
/// A captured screenshot as it is recorded in the run manifest.
export interface CaptureImage extends GalleryImage {
  platform: CapturePlatform
  model: string
}

/// The per-runner manifest fields recorded next to the shared git provenance.
...
/// The capture session handed to a platform runner.
export interface Capture {
  output: string
  command: (program: string, args: readonly string[]) => Promise<string>
  build: (args: readonly string[], logfile: string, appearance?: string) => Promise<void>
  exportImages: (
    result: string,
    key: string,
  ...
  finish: (metadata: CaptureMetadata) => Promise<void>
}

/// The local HTTP endpoint the XCTest runner posts window IDs to.
interface WindowCaptureServer {
  url: string
  stop: () => Promise<void>
}
...
export async function createCapture(
  ...
  async function build(args: readonly string[], logfile: string, appearance = "light") {
    ...
    try {
      ...
      // runXcodebuild rejects only with an Error.
      throw new Error(`${(error as Error).message}. See ${join(output, logfile)}`, { cause: error })
    } finally {
      await capture?.stop()
      await log.close()
    }
  ...
  async function finish(metadata: CaptureMetadata) {
    const manifest = {
...
async function captureMacWindows(directory: string): Promise<WindowCaptureServer> {
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      // Every throw above, including JSON.parse and screencapture, is an Error.
      response.writeHead(500, { "Content-Type": "text/plain" }).end((error as Error).message)
    }
  })
  ...
  return {
    // A listening TCP server always reports an AddressInfo, never a pipe path.
