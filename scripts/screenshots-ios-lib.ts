import type { CaptureHelpRequest, IosCaptureOptions, ScreenshotDevice } from "./screenshots-lib.ts"
import { parseCaptureOptions } from "./screenshots-lib.ts"
export { scenes, gallery, pngDimensions, screenshotAttachments } from "./screenshots-lib.ts"

/// An App Store screenshot slot bound to the simulator device type that fills it.
export interface SimulatorDevice extends ScreenshotDevice {
  type: string
}
...
/// The `simctl list runtimes --json` entry fields runtime selection reads.
export interface SimulatorRuntime {
  identifier: string
  name: string
  version: string
  isAvailable: boolean
  supportedDeviceTypes: readonly SimulatorDeviceType[]
}
...
} satisfies Record<string, SimulatorDevice>
...
export function selectRuntime(
  ...
  selectedDevices: readonly SimulatorDevice[],
  requested?: string
): SimulatorRuntime {
