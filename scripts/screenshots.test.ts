...
import {
  ...
  pngDimensions,
  selectedAppearances
} from "./screenshots-lib.ts"
import type { CaptureOptions } from "./screenshots-lib.ts"
...
test("both platforms capture light and dark by default, or one explicit appearance", () => {
  for (const platform of ["ios", "macos"] as const) {
    ...
    const options = parseCaptureOptions([], "/checkout", platform) as CaptureOptions
    ...
    for (const appearance of ["light", "dark"]) {
      assert.deepEqual(
        selectedAppearances(
          parseCaptureOptions(["--appearance", appearance], "/checkout", platform) as CaptureOptions
        ),
        [appearance]
      )
    }
