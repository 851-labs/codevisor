...
if (tapDir === undefined || tapDir.length === 0) {
  throw new Error("usage: update-homebrew-tap.ts <tap-dir>")
}
...
const sha256 = (file: string): string =>
  createHash("sha256")
    ...
    .digest("hex")
const releaseUrl = (file: string): string => `${artifactBaseUrl.replace(/\/$/, "")}/${file}`
const updateRenameFile = (filename: string, oldName: string, newName: string): void => {
  const path = join(tapDir, filename)
  const renames: Record<string, string> = existsSync(path)
...
const caskArtifactStanza =
  ...
  url "${releaseUrl("Codevisor-macOS-#{arch}.zip")}"`
    : // The guard above rejects a release with neither a universal app zip nor
      // both architecture-specific ones, so this branch always has appZip.
      `version "${version}"
...
const targetBlock = (target: string): string | undefined => {
