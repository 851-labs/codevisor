...
import type {
  AppStoreClient,
  AppStoreRequestOptions,
  BetaGroupResource,
  BuildListResponse,
  BuildQuery,
  BuildResource,
  DeliverInternalBuildOptions,
  ResourceIdentifier
} from "./app-store-connect.ts"
...
const build: BuildQuery = { appId: "app", version: "1.2.3", buildNumber: "42" }
const validBuild: BuildResource = {
  id: "build",
  type: "builds",
...
const group: BetaGroupResource = {
  id: "group",
  type: "betaGroups",
  attributes: { name: "Alpha", isInternalGroup: true }
}

function buildResponse(internalBuildState = "READY_FOR_BETA_TESTING"): BuildListResponse {
  return {
    data: [validBuild],
    included: [
...
// The client type is generic over the payload each caller reads, so no double
// can implement it: every fake below answers just the requests its test makes
// and is asserted into the client type.
interface FakeRequest extends AppStoreRequestOptions {
  query: Record<string, string>
}

// Group creation is the only request body a fixture reads members out of.
interface CreateGroupRequest extends FakeRequest {
  body: { data: { attributes: unknown; relationships: { app: { data: ResourceIdentifier } } } }
}

interface DeliveryFixtureOptions {
  post?: (attempt: number, options: AppStoreRequestOptions) => unknown
  membership?: (read: number) => boolean | Promise<boolean>
}

function deliveryFixture({ post = () => {}, membership }: DeliveryFixtureOptions = {}) {
  let assigned = false
  const counts = { posts: 0, reads: 0 }
  const client = async (path: string, options = {} as FakeRequest) => {
  ...
  const run = (options: DeliverInternalBuildOptions) =>
    deliverInternalBuild(
      client as unknown as AppStoreClient,
      build,
...
test("API authentication creates an ES256 token with Apple's audience and a bounded lifetime", () => {
  ...
  // JSON.parse stringifies its argument, so the decoded buffer is the JSON text.
  const decode = (value: string) => JSON.parse(Buffer.from(value, "base64url") as unknown as string)
...
test("app preflight queries the exact bundle and exposes authorization failures", async () => {
  const client = appStoreClient(credentials, {
    ...
    // The client always fetches a URL and sends a headers object; the fake
    // implements that subset of fetch's signature.
    fetchImplementation: (async (url: URL, options: { headers: Record<string, string> }) => {
  ...
  await assert.rejects(
    findApp((async () => ({ data: [] })) as unknown as AppStoreClient, "com.example.app"),
    /team and app access/
  )
})
...
test("readiness waits for the matching beta detail after processing becomes valid", async () => {
  ...
  const result = await waitForBuild(
    (async (path: string, { query }: FakeRequest) => {
...
test("a build already in internal testing is ready without sleeping", async () => {
  const client = (async () => buildResponse("IN_BETA_TESTING")) as unknown as AppStoreClient
  const result = await waitForBuild(client, build, {
...
test("unusable internal testing states fail with an actionable reason", async () => {
  for (const state of ["PROCESSING_EXCEPTION", "EXPIRED", "MISSING_EXPORT_COMPLIANCE"]) {
    await assert.rejects(
      waitForBuild((async () => buildResponse(state)) as unknown as AppStoreClient, build, {
...
test("readiness timeout reports a valid build's missing beta detail", async () => {
  const client = (async () => ({ data: [validBuild] })) as unknown as AppStoreClient
  await assert.rejects(
    waitForBuild(client, build, { attempts: 1 }),
...
test("processing fails promptly on rejected, internal-only, unknown-audience, or expired builds", async () => {
  for (const [attributes, message] of [
    ...
    await assert.rejects(
      waitForBuild((async () => ({ data: [{ attributes }] })) as unknown as AppStoreClient, build, {
        sleep: async () => assert.fail("Terminal states must not sleep")
      }),
      message
    )
...
test("delivery uploads an App Store eligible build once and assigns only an internal group", async () => {
  ...
  let uploaded = false
  let assigned = false
  const client = async (path: string, options = {} as CreateGroupRequest) => {
  ...
  await deliverInternalBuild(client as unknown as AppStoreClient, build, async () => {
...
test("a persistent build 404 stops at the deadline and preserves Apple's error", async () => {
  ...
  await assert.rejects(fixture.run({ ...clock, assignmentTimeout: 40_000 }), (failure: Error) => {
...
test("a matching external group is never used for internal delivery", async () => {
  await assert.rejects(
    internalGroup(
      (async (_path: string, options = {} as FakeRequest) => {
