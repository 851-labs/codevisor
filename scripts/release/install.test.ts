...
/// The knobs one installer run varies: the fixture feed it is served, the
/// platform and identity it reports, and the environment the script runs under.
...
/// One line of requests.jsonl, as the curl stub records it.
...
const install = (t: TestContext, options: InstallOptions = {}) => {
...
`,
  ...
  return {
    ...
    root,
    runtime,
    requests: readFileSync(requests, "utf8")
      .trim()
      .split("\n")
      .filter(Boolean)
      .map((line): RecordedRequest => JSON.parse(line)),
...
const rejectedManifests: readonly [string, InstallOptions][] = [
...
for (const [name, options] of rejectedManifests) {
...
const pinnedVersions: readonly [string, Record<string, string>, string][] = [
...
for (const [name, env, expectedVersion] of pinnedVersions) {
