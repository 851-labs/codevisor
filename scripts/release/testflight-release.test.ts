...
import { AppStoreConnectError } from "./app-store-connect.ts"
import type { AppStoreClient, BetaGroupResource, ResourceIdentifier } from "./app-store-connect.ts"
import { promoteTestFlightBuild, testFlightReleaseNotes } from "./testflight-release.ts"
import type {
  BetaAppLocalizationResource,
  BetaAppReviewAttributes,
  BetaBuildLocalizationResource,
  BetaReviewSubmissionResource,
  PromoteTestFlightOptions
} from "./testflight-release.ts"

// Apple returns complete resources; a fixture only supplies the attributes the
// promotion reads back, so the identifiers around them stay optional.
type Stub<Resource extends { attributes: unknown }> = Omit<Partial<Resource>, "attributes"> &
  Pick<Resource, "attributes">

// The fixture answers only the endpoints the promotion calls, so it declares
// the request subset those calls carry rather than Apple's full option shape;
// every mutation it sees writes group or localization attributes.
interface FixtureRequest {
  method?: string
  query: Record<string, string>
  body: {
    data: {
      attributes: BetaGroupResource["attributes"] & BetaBuildLocalizationResource["attributes"]
      relationships: Record<string, { data: ResourceIdentifier }>
    }
  }
}
...
interface FixtureState {
  ...
  autoNotify: boolean
  assigned: boolean
  review: BetaAppReviewAttributes
  localizations: BetaAppLocalizationResource[]
  submission?: Stub<BetaReviewSubmissionResource>
  group?: Stub<BetaGroupResource>
  legacyGroup?: Stub<BetaGroupResource>
  localization?: Stub<BetaBuildLocalizationResource>
  submit?: (state: FixtureState) => { data: Stub<BetaReviewSubmissionResource> }
}
...
function fixture(overrides: Partial<FixtureState> = {}) {
  ...
  const requests: (FixtureRequest & { path: string })[] = []
  const resource = () => ({
    id: "build",
    type: "builds",
    attributes: {
  ...
  // Apple's client omits options entirely for plain reads; the fixture only
  // inspects the query and body of the requests that do carry them.
  const client = async (path: string, options = {} as FixtureRequest) => {
  ...
  // The fixture answers a fixed set of endpoints instead of reproducing the
  // client's generic per-call result type, so it stands in for it by cast.
  const run = (options: PromoteTestFlightOptions = {}) =>
    promoteTestFlightBuild(client as unknown as AppStoreClient, configuration, {
      notes,
...
test("invalid or incomplete release setup fails before any mutations", async () => {
  const setups: [Partial<FixtureState>, RegExp][] = [
  ...
  for (const [overrides, message] of setups) {
