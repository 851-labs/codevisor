...
// Node derives a public key from a private KeyObject, but the bundled
// @types/node leaves KeyObject out of createPublicKey's parameter union.
type PublicKeySource = Parameters<typeof createPublicKey>[0]

const decodeSeed = (secret: string): Buffer => {
...
export const signSparkleUpdate = (
  contents: Buffer,
  secret: string,
  ...
  const publicDER = createPublicKey(privateKey as unknown as PublicKeySource).export({
    format: "der",
    type: "spki"
  })
  ...
  if (
    !verifySignature(
      null,
      contents,
      createPublicKey(privateKey as unknown as PublicKeySource),
      signature
    )
  ) {
...
if (process.argv[1] === import.meta.filename) {
  ...
  if (!path || !publicKey || !privateKey) {
    console.error(
      "usage: SPARKLE_PRIVATE_KEY=... sign-sparkle-update.ts <archive-path> <public-key>"
    )
    process.exit(1)
  }
