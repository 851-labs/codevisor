...
// The Manifest V3 fields this build reads or rewrites for the Store listing.
...
const usage = (): void => {
  console.error("usage: scripts/release/build-browser-extension.ts <version> [output-dir]")
  console.error()
...
// The argument count check above guarantees a first argument.
...
try {
  ...
  const manifest: ExtensionManifest = JSON.parse(readFileSync(manifestPath, "utf8"))
  ...
  // Always Node, never process.execPath: --check is a Node-only flag that parses
  // without executing. Bun has no equivalent — `bun --check <file>` RUNS the file.
  // Hardcoding the interpreter keeps this a syntax check even when this script is
  // itself invoked under Bun.
