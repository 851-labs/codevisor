...
import {
  ...
  uploadPackage,
  waitForUpload
} from "./publish-browser-extension.ts"
...
/// One recorded request. `fetch` types its arguments as wide unions, but the
/// subject always passes a URL string and a plain options object literal, which
/// is the shape these assertions read.
...
test("fetchStatus calls the Chrome Web Store v2 item endpoint", async () => {
  ...
  const status = await fetchStatus(configuration, async (url, options) => {
    calls.push({ url, options } as unknown as RecordedRequest)
