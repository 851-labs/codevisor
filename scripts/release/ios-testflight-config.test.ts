...
import type { TestFlightConfiguration } from "./ios-testflight-config.ts"
...
test("temporary API key is private and removed even when signing fails", async () => {
  ...
  await assert.rejects(
    // withSigningKey reads only the private key and its identifier off the
    // configuration, so the fixture implements that subset.
    withSigningKey(
      { privateKey: "test key", keyId: "EXAMPLEKEY" } as unknown as TestFlightConfiguration,
      async (path) => {
        keyPath = path
