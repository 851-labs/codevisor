...
import { verifyAlphaProvenance } from "./ios-testflight-config.ts"
import type { AlphaProvenance, AlphaWorkflowRun } from "./ios-testflight-config.ts"

const [runId, artifactDirectory] = process.argv.slice(2)
const repository = process.env.GITHUB_REPOSITORY
if (runId === undefined || !/^[1-9]\d*$/.test(runId) || !artifactDirectory)
  throw new Error("Usage: prepare-ios-testflight.ts ALPHA_RUN_ID ARTIFACT_DIRECTORY")
...
const run: AlphaWorkflowRun = JSON.parse(gh("api", `repos/${repository}/actions/runs/${runId}`))
...
const provenance: AlphaProvenance = JSON.parse(
  await readFile(join(directory, "release-provenance.json"), "utf8")
)
