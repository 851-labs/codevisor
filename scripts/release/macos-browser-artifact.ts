...
// Runs one signing or inspection command and returns its combined output.
...
export async function embeddedLibraries(app: string): Promise<string[]> {
...
export async function signEmbeddedLibraries(
  app: string,
...
function browserExecutables(app: string): [string, ...string[]] {
  return [
...
function architecturesOf(binary: string, run: CommandRunner): string[] {
  return run("lipo", ["-archs", binary]).trim().split(/\s+/)
}

export function verifyBrowserLinkage(
  app: string,
...
export async function verifyBrowserDistribution(
  app: string,
  architectures: readonly string[],
  run: CommandRunner = execute
): Promise<void> {
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (!app)
    throw new Error(
      "Usage: macos-browser-artifact.ts <linkage|distribution|sign-libraries> <app> <architectures...|identity>"
    )
