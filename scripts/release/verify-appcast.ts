...
export const verifyAppcast = (contents: string, build: string, expectedChannel: string): void => {
  ...
  // The length check above leaves exactly one matching item.
...
if (process.argv[1] === import.meta.filename) {
  ...
  if (!path || !build || !channel) {
    console.error("usage: verify-appcast.ts <appcast-path> <build> <alpha|stable>")
    process.exit(1)
  }
