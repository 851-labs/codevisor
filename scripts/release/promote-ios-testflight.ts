...
import type { TestFlightBuildRecord } from "./ios-testflight-config.ts"
...
if (!version || !artifactDirectory || !notesPath || (mode && mode !== "--check"))
  throw new Error(
    "Usage: promote-ios-testflight.ts VERSION ARTIFACT_DIRECTORY RELEASE_NOTES [--check]"
  )
...
const record: TestFlightBuildRecord = JSON.parse(
  await readFile(join(directory, "testflight-build.json"), "utf8")
)
