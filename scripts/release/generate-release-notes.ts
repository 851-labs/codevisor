...
if (
  !(["alpha", "stable"] as readonly (string | undefined)[]).includes(channel) ||
  !version ||
  !output
) {
  throw new Error(
    "usage: generate-release-notes.ts --channel <alpha|stable> --version <version> --commit <sha> --output <path>"
  )
}
...
const records = git("log", "--no-merges", "--format=%H%x09%s", range)
  ...
  .filter(Boolean)
  .map((line) => {
    // split() always yields a first field, even for an empty line.
    const [sha, ...subjectParts] = line.split("\t") as [string, ...string[]]
...
const groups: Record<"Added" | "Fixed" | "Changed", string[]> = {
  Added: [],
  Fixed: [],
  Changed: []
}
