...
import { chromiumHelperSuffixes } from "../chromium-artifact.ts"
import {
  embeddedLibraries,
  signEmbeddedLibraries,
  verifyBrowserDistribution,
  verifyBrowserLinkage
} from "./macos-browser-artifact.ts"
import type { CommandRunner } from "./macos-browser-artifact.ts"

const storage = "CodevisorBrowserStorage.dylib"
const loadPath = `@rpath/${storage}`
const arches = ["arm64", "x86_64"]

/// One recorded invocation of codesign, lipo or xcrun.
interface RecordedCommand {
  command: string
  args: readonly string[]
}

/// Paths and slices the inspection fixture should report as faulty.
interface InspectionFaults {
  omitLink?: string
  missingArch?: string
  noTimestamp?: string
...
function inspection({
  ...
  const run: CommandRunner = (command, args) => {
    ...
    // Every inspected command ends with the path it addresses.
...
test("release signing includes top-level libraries without touching sealed frameworks or aliases", async (t) => {
  ...
  const calls: RecordedCommand[] = []
  // Signing ignores the runner's output here, so the recorder returns nothing
  // rather than standing in for a full command runner.
  await signEmbeddedLibraries(app, "Developer ID Application: Fixture", ((
    command: string,
    args: readonly string[]
  ) => {
    calls.push({ command, args })
...
test("ad-hoc packaging stays explicit and does not request a timestamp", async (t) => {
  ...
  // As above: only the arguments matter, so the recorder skips the output.
  await signEmbeddedLibraries(app, "-", ((_: string, args: readonly string[]) =>
    calls.push(args)) as unknown as CommandRunner)
