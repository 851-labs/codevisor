...
import { appStoreClient, findApp } from "./app-store-connect.ts"
import { prepareEmbeddedCode, verifyDistributionSignatures } from "./ios-code-signing.ts"
import {
  ...
  withSigningKey
} from "./ios-testflight-config.ts"
import type { ExportOptions } from "./ios-testflight-config.ts"

// Xcode records one entry per exported artifact in DistributionSummary.plist.
interface DistributionSummary {
  "Codevisor.ipa": [{ entitlements: ExportedEntitlements }]
}

interface ExportedEntitlements {
  "get-task-allow"?: boolean
  "beta-reports-active"?: boolean
...
// Each plist has its own shape, so every caller declares the one it reads.
const plist = <Result>(path: string): Result =>
  JSON.parse(execFileSync("plutil", ["-convert", "json", "-o", "-", path], { encoding: "utf8" }))
const info = plist<Record<string, unknown>>(join(bundle, "Info.plist"))
...
const sourcePrivacy = plist<unknown>(join(repoRoot, "apps/ios/Codevisor/PrivacyInfo.xcprivacy"))
const exportedPrivacy = plist<unknown>(join(bundle, "PrivacyInfo.xcprivacy"))
...
const summary = plist<DistributionSummary>(join(exported, "DistributionSummary.plist"))[
  "Codevisor.ipa"
][0]
...
const actualOptions = plist<Partial<ExportOptions>>(join(exported, "ExportOptions.plist"))
if (
