...
/// The production-shaped roots one dev server owns: `.codevisor` state plus
/// the sibling worktrees directory.
export interface DevelopmentRoots {
  root: string
  data: string
  logs: string
  repos: string
...
export interface DevelopmentDerivedDataPaths {
  derivedData: string
  sourcePackages: string
}

export interface DevelopmentBuildPaths {
  root: string
  macos: DevelopmentDerivedDataPaths
  ios: DevelopmentDerivedDataPaths
  pixelbook: DevelopmentDerivedDataPaths
  packageCache: string
  bunCache: string
  nodeGyp: string
  generated: string
...
export interface DevelopmentRuntimePaths {
  root: string
  temp: string
  manifest: string
}

export interface DevelopmentLayout {
  tmpRoot: string
  local: DevelopmentRoots
  remote: DevelopmentRoots
  remoteCloud: DevelopmentRoots
  build: DevelopmentBuildPaths
  wrangler: string
  runtime: DevelopmentRuntimePaths
}

export const IOS_DEVELOPMENT_BUNDLE_IDENTIFIER = "com.851labs.Codevisor.Development.iOS"

export function iosDevelopmentBundleIdentifier(repoRoot: string): string {
...
export async function ensureDevelopmentDirectories(layout: DevelopmentLayout): Promise<void> {
  await Promise.all(
    [
      layout.local.data,
      layout.local.logs,
...
export async function ensureBuildDirectories(layout: DevelopmentLayout): Promise<void> {
  await Promise.all(
...
export function localDevelopmentEnvironment(
...
export function remoteDevelopmentEnvironment(
  layout: DevelopmentLayout,
  environment: NodeJS.ProcessEnv = process.env,
  remote: DevelopmentRoots = layout.remote
): NodeJS.ProcessEnv {
  return {
    ...environment,
    TMPDIR: layout.runtime.temp,
