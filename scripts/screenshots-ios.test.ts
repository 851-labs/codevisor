...
import {
  ...
  selectRuntime
} from "./screenshots-ios-lib.ts"
import type { SimulatorDevice, SimulatorRuntime } from "./screenshots-ios-lib.ts"
import type { IosCaptureOptions } from "./screenshots-lib.ts"

test("options stay rooted in this checkout and reject missing or unknown arguments", () => {
  // Only `--help` short-circuits parsing, so these calls always yield options.
  const parse = (args: readonly string[]) => parseOptions(args, "/checkout") as IosCaptureOptions
  assert.equal(parse([]).output, "/checkout/tmp/screenshots/ios")
  assert.equal(parse([]).device, "iphone")
  assert.equal(parse(["--device", "all"]).device, "all")
  assert.deepEqual(
    parseOptions(
...
test("runtime selection requires supported devices and ignores unavailable runtimes", () => {
  const runtime = (
    version: string,
    supported: readonly SimulatorDevice[] = Object.values(devices),
    isAvailable = true
  ): SimulatorRuntime => ({
    name: `iOS ${version}`,
