...
import { chromiumHelperName, chromiumHelperSuffixes, run } from "./chromium-artifact.ts"

/// The Xcode build-phase variables this script reads. Xcode always sets them
/// for the "Embed Chromium" phase that runs this file, so the contract is
/// declared once here instead of guarded at every read.
interface ChromiumEmbedEnvironment extends NodeJS.ProcessEnv {
  ARCHS: string
...
const env = process.env as ChromiumEmbedEnvironment
// Xcode always sets ARCHS to at least one architecture, which is what the
// architectures[0] assertions below rely on.
...
async function* files(directory: string, relative = ""): AsyncGenerator<string> {
...
const devToolsResources: Record<string, number> = Object.fromEntries(
...
// `as const` keeps each pair a two-string tuple instead of string[].
