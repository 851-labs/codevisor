...
import { createCapture } from "./screenshots-capture.ts"
import { devices, parseOptions, selectRuntime } from "./screenshots-ios-lib.ts"
import type { SimulatorRuntime } from "./screenshots-ios-lib.ts"
import { selectedAppearances } from "./screenshots-lib.ts"
...
const simctl = (...args: string[]): Promise<string> => command("xcrun", ["simctl", ...args])
const selected = Object.entries(devices).filter(
  ([key]) => options.device === "all" || options.device === key
)
const { runtimes }: { runtimes: SimulatorRuntime[] } = JSON.parse(
  await simctl("list", "runtimes", "--json")
)
