...
// machines as scripts/dev.ts (Linux containers by default), starts a
...
} from "./dev-containers.ts"
import {
  buildIOSDevelopmentApp,
  launchIOSDevelopmentApp,
  terminateIOSDevelopmentApp
} from "./dev-ios-target.ts"
import type { IOSDevelopmentTarget } from "./dev-ios-target.ts"
import {
  developmentLayout,
...
// Shared with scripts/dev.ts's dev servers so the simulator talks to the
...
// Same hash → hue derivation as scripts/dev.ts, so a worktree's iOS icon
...
const directRemoteEnvironment: NodeJS.ProcessEnv = {
...
let stopping = false
let iosTarget: IOSDevelopmentTarget | undefined
...
const watchServerExit = (child: ChildProcess, label: string): Promise<void> =>
...
async function readConnectionToken(): Promise<string> {
  try {
...
/// The dev-cloud session shared with the standalone server and the app.
interface DevCloudSession {
  url: string
...
async function resolveCloudSession(
  cloudUrl: string,
  ownedCloud: ChildProcess | undefined
): Promise<DevCloudSession | undefined> {
...
async function readCloudDevVariables(): Promise<Record<string, string>> {
