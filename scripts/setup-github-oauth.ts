...
// listen() above resolved, so address() is a bound AddressInfo, never null/string.
const port = (server.address() as AddressInfo).port
const redirectUrl = `http://127.0.0.1:${port}/callback`

const done = new Promise<CreatedGitHubApp>((resolveDone, rejectDone) => {
  server.on("request", async (request, response) => {
    ...
    if (url.pathname === "/callback") {
      try {
        ...
        const app = (await converted.json()) as CreatedGitHubApp
        ...
        if (options.prod) {
          ...
          const content =
            existing !== undefined
              ? `${existing.trimEnd()}\n${block}`
              : `# Created by scripts/setup-github-oauth.ts (${app.html_url})\n${block}`
        ...
        response.end(`Setup failed: ${error instanceof Error ? error.message : String(error)}`)
        rejectDone(error)
      }
      return
    }
