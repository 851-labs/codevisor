...
/// The assembled copy, plus whether this call refreshed it.
export interface LinuxWorkspace {
  appRoot: string
  changed: boolean
}

const copyIfPresent = async (from: string, to: string): Promise<boolean> => {
  if (!(await pathExists(from))) return false
  await cp(from, to, { recursive: true, force: true })
  return true
}

const workspacePackageDirectories = async (repoRoot: string): Promise<string[]> => {
...
const workspaceSignature = async (
  repoRoot: string,
...
const newestMtime = async (path: string): Promise<number> => {
...
export async function syncLinuxWorkspace(
  repoRoot: string,
  containerRoot: string
): Promise<LinuxWorkspace> {
