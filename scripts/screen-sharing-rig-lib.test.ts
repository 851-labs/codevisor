...
import type { RigConfiguration, RigRole } from "./screen-sharing-rig-lib.ts"
...
test("rig configuration validates roles, tokens, peers and capture", () => {
  ...
  // rigConfiguration guards roles that never passed through the union: the
  // value arrives unvalidated from the command line.
  assert.throws(() => rigConfiguration({ role: "admin" as unknown as RigRole, token }), /role/)
...
test("tune accepts a JSON object, the product profile, or default", () => {
  // withTuning only reshuffles codec, bitrate and tuning, so the fixtures
  // implement that subset of a configuration.
  assert.deepEqual(
    withTuning(
      { role: "host", codec: "h264", tuning: { drawables: 2 } } as unknown as RigConfiguration,
  ...
  assert.deepEqual(
    withTuning(
      { role: "host", codec: "hevc444", tuning: { drawables: 2 } } as unknown as RigConfiguration,
      { bitrate: 24_000_000 }
    ),
  ...
  const base: RigConfiguration = { role: "host", token: "t", tuning: { profile: "paced15-worker" } }
