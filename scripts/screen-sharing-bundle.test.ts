...
test("falls back to ad hoc with a warning when no development identity exists", () => {
  ...
  // The ad-hoc branches always attach a warning; the field is optional only
  // because the signed paths leave it off.
  assert.match(
    resolved.warning as string,
    /^WARNING: signing ad hoc \(no Apple Development identity found\)/
  )
  assert.match(resolved.warning as string, new RegExp(signingIdentityVariable))
})
...
test("environment override wins, may be a hash, and rejects revoked identities", () => {
  ...
  assert.match(explicitAdHoc.warning as string, /requested through the environment/)
})
