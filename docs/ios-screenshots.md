...
This builds the actual Debug app and captures four native screens on an iPhone 13 Pro Max and a 13-inch iPad Pro, in this order: projects, conversation, new chat sheet, and browser preview. The portfolio workspace includes a Claude chat in progress. The browser contains a bundled example project. All content is fixed, offline demo data; no account, API key, AI session, or development server is needed.

Each run writes a new `tmp/screenshots/ios/capture-*` directory containing:

- `iphone/` and `ipad/`: full-resolution portrait PNGs with device suffixes, such as `01-projects-iphone.png` and `01-projects-ipad.png`.
...
The iPhone PNGs are **1284 × 2778**, accepted in App Store Connect’s **6.5-inch iPhone** slot. The iPad PNGs are **2064 × 2752** for the **13-inch iPad** slot. The script validates every exported image against these exact dimensions and fails on a mismatch. See [Apple’s screenshot specifications](https://developer.apple.com/help/app-store-connect/reference/app-information/screenshot-specifications).
...
For a quicker iteration or a specific installed runtime:

```sh
bun run screenshots:ios --device iphone
bun run screenshots:ios --device ipad --runtime 'iOS 27.0'
