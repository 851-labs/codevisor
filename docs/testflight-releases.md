...
Alpha uploads an App Store eligible iOS build and assigns it only to the internal
Alpha group. Public TestFlight publication uses the separate, manually triggered
**Publish iOS TestFlight** workflow. Normal macOS/server releases do not submit
an iOS build for external testing.

## Submit an iOS beta

In GitHub Actions, open **Publish iOS TestFlight**, choose **Run workflow** on
**main**, and enter the successful **Alpha run ID** containing the iOS build you
...
The default external group is **Public Beta**. The job creates it if missing.
...
The job uses the same four Apple signing/API secrets as Alpha. No new secrets
...
Supply the same Apple environment variables as Alpha, plus
