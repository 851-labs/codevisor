...
- **Xcode's macro trust gate.** These are the project's first package dependencies that ship macro plugins, and `xcodebuild` refuses unapproved macros. Both command-line entry points (`scripts/xcodebuild.ts` for `build:macos`/`build:ios`/`dev`, and `scripts/release/build-macos-xcode.sh`) pass `-skipMacroValidation`, trusting the pinned checkouts. Opening the project in Xcode itself asks once.
