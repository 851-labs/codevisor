...
- **Hold `PreventUserIdleDisplaySleep` while a session is live** (done in 851-2375) (`IOPMAssertionCreateWithName`), released when the session ends. Apple's Screen Sharing behaves this way. An explicit `displaysleepnow`, lid close or lock still stops the displays; the assertion only removes idle sleep as a cause of a silent host.
- **Restart capture on a stream error instead of ending the session** (done in 851-2375: after 1 s, at most 3 times a minute, with "Capture stopped, restarting…" shown over the paused picture), bounded (for example every 5 s), and end the session only after a bounded number of failures. The rig measured recovery within a second of wake with this policy, on the same WebRTC session, with no viewer action. This also covers display reconfiguration where the selected display survives.
- **Treat "started, no frames" as a distinct state.** (Done in 851-2385: the host restarts the capture, then `replayd`, and the viewer shows "Capture stalled, restarting…" meanwhile.) After a capture starts, no delivered frame within a few seconds should set a visible host state ("capture stalled") and log it, rather than showing a connected session at 0 fps. The rig's `sourceStall` label is the shape.
